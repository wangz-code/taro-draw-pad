...
export default class SignaturePad {
	...
	fromDataURL = (dataUrl, options = {}, callback) => {
		...
		const image = options.wx ? this.canvas.createImage() : new Image();
		const ratio = options.ratio || options.wx ? Taro.getSystemInfoSync().pixelRatio : 1;
	...
	setCtx = (opt) => {
		this.penColor = opt.penColor;
		this.strokeWidth = undefined;
		if (opt.isSignature) this.maxWidth = opt.strokeWidth;
		if (!opt.isSignature) this.strokeWidth = opt.strokeWidth;
	};
	...
	_calculateCurveWidths(startPoint, endPoint) {
		...
		const newWidth = this.strokeWidth || this._strokeWidth(velocity);
		const widths = {
			end: newWidth,
